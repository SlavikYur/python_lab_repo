...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://iotstudent:4321@localhost/iot_test_db'
...
def get_items_ordered():
    ...
    result = items_ordered_schema.dump(all_items)
...
def item_ordered_info(id):
    ...
    return items_ordered_schema.jsonify(item)


@app.route('/item_ordered/<id>', methods=['PUT'])
def update_item_ordered():
    item = ItemOrdered.query.get(id)

    name = request.json['name']
    price = request.json['price']
    weight_kg = request.json['weight']
    item_count = request.json['item_count']
    item_type = request.json['item_type']
